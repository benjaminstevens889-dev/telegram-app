      ...
      <div ref={messagesContainerRef} onScroll={handleScroll} className="flex-1 overflow-y-auto p-4 custom-scrollbar">
        <div className="space-y-3">
          {groupMessages.map((msg) => {
            ...
            return (
              <div key={msg.id} className={`flex ${isOwn ? 'justify-start' : 'justify-end'} group items-center gap-2`}>
                {isOwn && !isDeleting && (
                  <button onClick={() => setMessageToDelete(msg)} className="opacity-0 group-hover:opacity-100 transition-opacity p-1.5 rounded-full bg-red-500/80 hover:bg-red-500">
      ...
      <div className="p-4 border-t border-white/5 bg-[#12121a]/80 backdrop-blur-xl">
        {isRecording ? (
          ...
          <div className="flex items-center gap-3">
            ...
            />
            <Button onClick={handleSend} className="modern-gradient hover:opacity-90 w-11 h-11 rounded-xl btn-press">
...
function ChatList({ 
  ...
  useEffect(() => {
    ...
    const pollInterval = setInterval(fetchRequests, 4000);
  ...
  useEffect(() => {
    ...
    fetchGroups();
    const poll = setInterval(fetchGroups, 4000);
    return () => clearInterval(poll);
  }, [setGroups]);
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const pollInterval = setInterval(fetchChats, 4000);
